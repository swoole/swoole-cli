const show_git_branch_list = async () => {
    const response = await fetch('/api/branchList', {
        credentials: 'include',
        mode: 'cors',
        method: 'GET',
        headers: {
    ...
    if (response.status === 200 && res) {
        const branch_list = document.querySelector('select[name="branch_list"]')
        if (branch_list) {
...
const change_branch = async (event) => {
    // console.log(event, event.target, event.target.value)
    const url = '/api/changeBranch'
